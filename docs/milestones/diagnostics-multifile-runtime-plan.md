...
| 6. Nicole stack trace | in_progress | `700654d9e99225cff9337b16bb2f08a797a0447a` | Phase 6C completed: RuntimeStackTrace is threaded internally through runtime execution and frame lifecycle attachment is implemented at frozen creation points | `226 passed`; `850 passed` | WORD frames in `_invoke_word(...)`; HOST frames in `_execute_host_call(...)`; QUOTATION frames only in `_invoke_runtime_quote_value(...)`; `_execute_call(...)` avoids duplicate quotation frame; traces remain internal only; no diagnostics/runtime-error attachment; no renderer changes; no locals snapshots; no frame history; self-tail-call compactness preserved; runtime behavior preserved; Phase 6D is next |
...
- preserve existing renderer behavior unless explicitly extended later
- no ANSI formatting
- no locals snapshots
- no frame history
